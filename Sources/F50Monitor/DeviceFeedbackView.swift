...
public struct DeviceFeedbackView: View {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if isProcessing {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if let issueURLStr = createdIssueURL, let issueURL = URL(string: issueURLStr) {
                                Link(destination: issueURL) {
                                    HStack(spacing: 4) {
                                        Image(systemName: "link")
                                        Text("在 GitHub 查看自动生成的 Issue 进度")
                                    }
                    ...
                    if let currentReport = report {
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                Text("探测结果：有效接口 \(currentReport.successfulProbesCount) / \(currentReport.endpoints.count)")
                                    .font(.system(size: 11, weight: .medium))
                                    .foregroundColor(.secondary)
                                Spacer()
                                Button("复制诊断报告") {
                                    copyReportToClipboard(currentReport)
                                }
                                .buttonStyle(.plain)
                                .font(.system(size: 11))
                                .foregroundColor(F50Theme.blue)
                    ...
                    HStack(spacing: 5) {
                        Image(systemName: "lock.shield.fill")
                            .font(.system(size: 10))
                            .foregroundColor(.secondary)
                        Text("本地自动脱敏：不发送密码、Token、完整 IMEI/IMSI 与短信内容")
    ...
    private func processAndAttachImage(_ image: NSImage) {
    ...
    private func startProbeAndSubmit() {
        ...
        Task {
            ...
            let reportResult = await DeviceDiagnosticProbe.shared.executeProbe(
                ...
                candidateTokens: [fetcher.ufiToken, fetcher.password].filter { !$0.isEmpty },
                appVersion: "2.1.1"
            ) { progress, status in
                Task { @MainActor in
