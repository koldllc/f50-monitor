...
- 修复信号详情 RSRP/RSRQ/SINR 无法获取（适配 F50 固件 network_information dump 行为）
- 修复 F50 频段显示（Nr_bands 兑底）与网络制式 SA/NSA 精确识别
- 套餐流量卡片布局优化：已用比例与重置天数移至进度条下方
- UFI-TOOLS 签名算法抽取与回归测试（真实设备抓包向量）
